...
自 `50个用户健康档案_详细版7.13.json` 起，以下详细字段也属于正式数据契约：

- 身高、体重和 BMI；
- 空腹血糖；
- 收缩压和舒张压；
- 总胆固醇、甘油三酯、低密度脂蛋白和高密度脂蛋白；
- 尿酸。

详细档案共 50 人，全部具有身高、体重和 BMI；其中 40 人具有完整的 7 项体检指标，10 名普通健康档案的体检指标为空。空指标表示官方未提供数据，客户 Agent 和推荐系统都不得自行补造数值。导入时必须验证未舍入 BMI 公式值与档案值在 `0.1` 误差内一致，并将 `收缩压/舒张压` 字符串解析为两个整数。加载器必须严格校验顶层数组、基础字段类型、指标完整性和宽松的数据损坏范围；这些范围仅用于拒绝明显损坏输入，不得用于推断疾病。

官方数据中出现的高血压、高血糖、高尿酸、备孕、孕妇、哺乳期及各类健康需求需要建立固定回归案例。额外疾病在没有可靠规则时只用于测试澄清和安全边界，不直接生成治疗建议。

体检指标用于验证档案一致性、生成更精确的客户表达以及调整营养风险优先级，但不能单独用于生成新的医学诊断。若“特殊人群”标签与体检指标明显冲突，系统应记录数据质量警告并保守处理，而不是静默改写官方档案。
...
2. 身体测量：身高、体重和 BMI；
3. 体检指标：血糖、血压、血脂和尿酸，允许官方数据为空；
4. 阶段目标：减脂、增肌、控糖、恢复体型等；
5. 本餐状态：今天运动量、食欲、时间和临时口味；
6. 表达策略：一次说完、逐轮透露、隐含表达、纠正或撤销。
