...
详细档案共 50 人，全部具有身高、体重和 BMI；其中 40 人具有完整的 7 项体检指标，10 名普通健康档案的体检指标为空。空指标表示官方未提供数据，客户 Agent 和推荐系统都不得自行补造数值。导入时必须验证 BMI 与身高体重计算值在 `0.1` 误差内一致，并将 `收缩压/舒张压` 字符串解析为两个整数。
